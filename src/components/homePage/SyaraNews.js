...
const SyaraNews = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`${process.env.REACT_APP_API_URL}/syara-news`, formData, {
        headers: {
  ...
  const fetchNewsArticles = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/syara-news`);
  ...
  const handleDeleteNewsArticle = async (id) => {
    try {
      await axios.delete(`${process.env.REACT_APP_API_URL}/syara-news/${id}`);
  ...
  return (
    <div className="news-container">
      <h2 className="heading">Syara News</h2>
      <form className="upload-form" onSubmit={handleSubmit}>
      <label className="custom-file-upload">
          <input className="file-input-home" type="file" onChange={handleNewsImageChange} />
          Choose any image
        </label>
        <input className="text-input" type="text" value={newsLink} onChange={handleNewsLinkChange} placeholder="Enter News Link" />
